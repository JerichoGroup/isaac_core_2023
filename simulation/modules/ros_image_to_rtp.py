...
# ==================== stream topic on rtp func ====================
def stream_topic_on_rtp(topic: str, host: str, video_port: int, meta_port: int) -> None:

    if not rclpy.ok():
        rclpy.init()

    node = RosImageToRTPNode(topic, host, video_port, meta_port)
